...
CREATE POLICY "Organization members can view commissions" ON public.staff_commissions
    FOR SELECT USING (
        organization_id IN (
...
CREATE POLICY "Organization members can insert commissions" ON public.staff_commissions
...
CREATE POLICY "Organization members can update commissions" ON public.staff_commissions
    FOR UPDATE USING (
        organization_id IN (
            SELECT organization_id FROM public.organization_users 
            WHERE user_id = auth.uid() AND is_active = true
