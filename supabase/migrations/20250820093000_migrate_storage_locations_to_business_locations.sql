...
FROM src s
JOIN LATERAL (
  -- Find an existing migrated location or create one per organization
  WITH found AS (
...
ALTER TABLE IF EXISTS public.inventory_levels
...
ALTER TABLE IF EXISTS public.inventory_cost_layers
  ADD CONSTRAINT inventory_cost_layers_location_id_fkey
  FOREIGN KEY (location_id) REFERENCES public.business_locations(id) ON DELETE CASCADE;

ALTER TABLE IF EXISTS public.inventory_consumptions
  ADD CONSTRAINT inventory_consumptions_location_id_fkey
  FOREIGN KEY (location_id) REFERENCES public.business_locations(id) ON DELETE CASCADE;
