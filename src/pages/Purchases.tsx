...
export default function Purchases() {
  ...
  const fetchPurchases = useCallback(async () => {
    try {
      ...
      setPurchases(((data as any[]) || []) as unknown as Purchase[]);
    } catch (err) {
  ...
  const openReceiveDialog = useCallback(async (purchaseId: string) => {
    try {
      ...
      // Load purchase items for this purchase
      const { data: items, error: iErr } = await supabase
        .from("purchase_items")
        .select(`id, item_id, quantity, unit_cost, total_cost, received_quantity, inventory_items (name)`) 
        .eq("purchase_id", purchaseId);
      if (iErr) throw iErr;
      setSelectedPurchaseItems(((items as any[]) || []) as unknown as PurchaseItem[]);
      ...
      toast({ title: "Error", description: "Failed to load receiving data", variant: "destructive" });
    }
  }, [toast]);

  const submitReceive = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    if (!receivePurchaseId) return;
    if (!receiveLocationId) {
      toast({ title: "Location required", description: "Please select a location to receive into", variant: "destructive" });
      return;
    }
    try {
      // Ensure the purchase has a receiving location
      await supabase
        .from("purchases")
      ...
      const updates = selectedPurchaseItems
        .map((it) => {
          const delta = Number(receiveQuantities[it.item_id] || 0);
          if (!delta || delta <= 0) return null;
          const newReceived = Math.min(
            Number(it.quantity || 0),
            Number(it.received_quantity || 0) + delta
          );
      ...
      for (const u of updates) {
        const { error } = await supabase
          .from("purchase_items")
          .update({ received_quantity: u.received_quantity })
          .eq("id", u.id);
        if (error) throw error;
      }
  ...
  }, [receivePurchaseId, receiveLocationId, selectedPurchaseItems, receiveQuantities, fetchPurchases, toast]);

  const undoReceiving = useCallback(async (purchaseId: string) => {
    try {
      ...
      for (const row of list) {
        if (Number(row.received_quantity || 0) > 0) {
      ...
      toast({ title: "Undone", description: "Receiving has been undone" });
      fetchPurchases();
    } catch (err) {
      console.error("Error undoing receiving:", err);
      toast({ title: "Error", description: "Failed to undo receiving", variant: "destructive" });
    }
  }, [fetchPurchases, toast]);
