  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id) {
        ...
        if (sendWhatsAppNotification && inserted?.id) {
          ...
          const uniqueStaffIds = [...new Set(form.serviceItems.map(item => item.staff_id).filter(Boolean))];
          uniqueStaffIds.forEach(staffId => {
            notifications.push(
              supabase.functions.invoke('send-staff-whatsapp', {
                body: {
                  appointment_id: inserted.id,
                  staff_id: staffId,
                  organization_id: organization.id
          ...
          try {
            ...
            const failedNotifications = results.filter(result => result.status === 'rejected' || result.value?.error);
            
            if (failedNotifications.length === 0) {
              toast.success("Appointment created and WhatsApp notifications sent to client and staff!");
            } else if (failedNotifications.length < notifications.length) {
              toast.warning("Appointment created. Some WhatsApp notifications failed to send.");
            } else {
              toast.error('Appointment created but WhatsApp notifications failed to send');
            }
          } catch (whatsappErr) {
              ...
              {!id && (
                <div className="sm:col-span-2 lg:col-span-3 xl:col-span-4">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <Label htmlFor="send-whatsapp-notification" className="flex items-center gap-2">
                      ...
                      Send WhatsApp notifications to client and assigned staff
                    </Label>
                  </div>
                </div>
              )}
