    ...
    try {
      ...
      if (id) {
        ...
        if (sendWhatsAppNotification && inserted?.id) {
          try {
            const { error: whatsappError } = await supabase.functions.invoke('send-appointment-whatsapp', {
              body: {
            ...
            if (whatsappError) {
              ...
              toast.error('Appointment created but WhatsApp notification failed to send');
            } else {
              toast.success("Appointment created and WhatsApp notification sent!");
            }
          } catch (whatsappErr) {
